...
### PR #22: First-Poll Posting Fix (Merged)
Fixed a bug where adding a new source would post dozens of articles instead of just the latest. The initial fix in PR #15 only affected fetching, but items already in the database would still get summarized and posted.

**Changes**:
- Added `has_source_posted_content()`, `get_most_recent_item_for_source()`, `mark_items_as_backfilled()` to repository
- Added `_handle_first_posting_backfill()` in pipeline.py to mark old items as backfilled before summarizing
- Only unsummarized items are backfilled (to not affect items already in the posting queue)

### PR #23: Handle YouTube Videos Without Transcripts (Pending)
YouTube videos without transcripts (or when transcript fetch fails) were getting stuck in the database forever because:
1. `get_unsummarized_content_items()` filtered out items where `raw_content` is None
2. `get_unposted_content_items()` filtered out items where `summary` is None

**Changes**:
- Removed `raw_content.isnot(None)` filter from `get_unsummarized_content_items()` in repository.py
- Modified `summarize_pending()` in pipeline.py to set an empty summary for items without content (instead of skipping them)
- Content poster already handles empty summaries by displaying "No summary available." (line 97: `summary = content_item.summary or "No summary available."`)
